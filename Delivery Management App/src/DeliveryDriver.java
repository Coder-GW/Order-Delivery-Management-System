...
public class DeliveryDriver {
    ...
    public boolean canAcceptJob(LocalDate deliveryDate) {
        ...
        long jobsOnDate = assignedJobs.stream()
                .filter(job -> job.getDeliveryDate() != null
                        && job.getDeliveryDate().equals(deliveryDate)
                        && ("Assigned".equalsIgnoreCase(job.getStatus()) || "In Transit".equalsIgnoreCase(job.getStatus())))
                .count();
    ...
    public boolean assignJob(DeliveryJob job) {
        ...
        if (job.getDeliveryDate() != null &&
                !canAcceptJob(job.getDeliveryDate())) {
            return false;
        }

        // DeliveryJob.assignDriver now returns boolean
        boolean assigned = job.assignDriver(driverId);
        if (assigned) {
    ...
    // --- New: create an in-house staff record (generates password, encrypts and upserts into inhouse_staff) ---
    // Returns the plaintext password if creation succeeded, otherwise null.
    public static String createInhouseStaff(String staffId, String name, String contactNumber) throws IOException, InterruptedException {
        if (staffId == null || staffId.trim().isEmpty()) throw new IllegalArgumentException("staffId is required");
        if (name == null) name = "";
        if (contactNumber == null) contactNumber = "";

        // normalize staff id
        String sid = staffId.trim();

        // generate plaintext password
        String plaintext;
        try {
            plaintext = PasswordGenerator.generatePassword(12);
        } catch (Exception e) {
            // fallback (shouldn't happen)
            plaintext = Long.toHexString(System.currentTimeMillis()).substring(0, 12);
        }

        // choose master passphrase (env or fallback to staff id)
        String master = System.getenv("PASSWORD_MASTER_KEY");
        if (master == null || master.isEmpty()) master = sid;

        // encrypt using existing helper (passphrase + salt = staff id)
        PasswordGenerator.EncryptionResult res;
        try {
            res = PasswordGenerator.encryptWithPassphrase(plaintext, master, sid);
        } catch (Exception ex) {
            // encryption failed
            return null;
        }
        String encrypted = res.base64Iv + ":" + res.base64Ciphertext;

        // build JSON for inhouse_staff table: staffid, name, contact_number, password
        StringBuilder sb = new StringBuilder();
        sb.append("{");
        sb.append("\"staffid\":\"").append(escapeJson(sid)).append("\",");
        sb.append("\"name\":\"").append(escapeJson(name)).append("\",");
        sb.append("\"contact_number\":\"").append(escapeJson(contactNumber)).append("\",");
        sb.append("\"password\":\"").append(escapeJson(encrypted)).append("\"");
        sb.append("}");
        String body = "[" + sb.toString() + "]";

        // upsert into lower-case table name so PostgREST finds it
        HttpResponse<String> resp = SupabaseClient.postUpsert("inhouse_staff", body, "staffid", null);
        if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
            return plaintext;
        }
        return null;
    ...
    // --- New: verify inhouse staff credentials by fetching encrypted password, decrypting and comparing ---
    public static boolean verifyInhouseCredentials(String staffId, String candidatePassword) {
        if (staffId == null || staffId.trim().isEmpty() || candidatePassword == null) return false;
        try {
            String encoded = URLEncoder.encode(staffId, StandardCharsets.UTF_8);
            String q = "inhouse_staff?select=password&staffid=eq." + encoded;
            HttpResponse<String> resp = SupabaseClient.get(q, null);
            if (resp.statusCode() < 200 || resp.statusCode() >= 300) return false;
            String body = resp.body();
            if (body == null || !body.trim().startsWith("[") || body.trim().length() <= 2) return false;
            String obj = body.trim();
            obj = obj.substring(1, obj.length() - 1).trim();
            String stored = extractJsonString(obj, "password");
            if (stored == null || stored.isEmpty()) return false;

            // stored format expected: base64Iv:base64Ciphertext
            String[] parts = stored.split(":");
            if (parts.length != 2) return false;
            String base64Iv = parts[0];
            String base64Ct = parts[1];

            String master = System.getenv("PASSWORD_MASTER_KEY");
            if (master == null || master.isEmpty()) master = staffId;

            String plain;
            try {
                plain = PasswordGenerator.decryptWithPassphrase(master, staffId, base64Iv, base64Ct);
            } catch (Exception ex) {
                return false;
            }
            if (plain == null) return false;

            byte[] a = plain.getBytes(StandardCharsets.UTF_8);
            byte[] b = candidatePassword.getBytes(StandardCharsets.UTF_8);
            return MessageDigest.isEqual(a, b);
        } catch (Exception e) {
            return false;
